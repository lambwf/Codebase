...
load_gcb_countries_luc <- function(sheet_blue,sheet_oscar,sheet_luce){
  ...
  data_gcb_luc <- gather(data_gcb_luc,key,value,blue,oscar,luce,mean) %>% 
    mutate(value=value/1000) %>% 
    mutate(value=value*(44/12)) 
  
  data_gcb_luc <- spread(data_gcb_luc,key,value) %>%
    mutate(units="GtCO2") %>% 
    select(country,iso,units,year,blue,oscar,luce,mean) 
